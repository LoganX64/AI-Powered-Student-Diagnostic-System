...
import { getStudent, getStudentAssignments, type StudentDetail, type StudentAssignment } from "@/services/coach.service";
...
export function CoachStudentDetailPage() {
  ...
  if (!student) {
    return (
      <SidebarProvider>
        <CoachSidebar />
        <SidebarInset>
          <CoachSiteHeader title="Student Detail" />
  ...
  return (
    <SidebarProvider>
      <CoachSidebar />
      <SidebarInset>
        <CoachSiteHeader title={student.name} />
        <div className="flex flex-1 flex-col gap-6 p-4 lg:p-6">
          ...
          <Button
            variant="outline"
            className="w-fit"
            onClick={() => navigate(`/coach/students/${studentId}/sqi`)}
          >
