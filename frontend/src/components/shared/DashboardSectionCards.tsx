...
import { getDashboardCounts, type DashboardCounts } from "@/services/dashboard.service";

function buildAdminCards(counts: DashboardCounts) {
  return [
    {
      title: "Total Coaches",
...
function buildCoachCards(counts: DashboardCounts) {
  return [
    {
      title: "My Students",
      value: String(counts.totalStudents),
...
export function DashboardSectionCards() {
  ...
  const cards = role === "admin" ? buildAdminCards(counts) : buildCoachCards(counts);
